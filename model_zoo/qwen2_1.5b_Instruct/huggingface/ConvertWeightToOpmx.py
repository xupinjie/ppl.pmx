...
def write_pmx_model(model_path, input_base_path):
    ...
    # xx_state_dict = torch.load("mm.pth")
    # print(xx_state_dict.keys())
    ...
    for layer_i in range(pmx_params_dict['num_layers']):
        ...
        state_dict.update({
            ...
            f"layers.{layer_i}.attention.wq.bias": hf_model_state_dict[f"model.layers.{layer_i}.self_attn.q_proj.bias"],
            f"layers.{layer_i}.attention.wk.bias": hf_model_state_dict[f"model.layers.{layer_i}.self_attn.k_proj.bias"],
...
def main():
    ...
    write_pmx_model(
        ...
        model_path="../models",
        input_base_path="/home/SENSETIME/xupinjie1/.cache/modelscope/hub/qwen/Qwen2-1___5B-Instruct"
    )
