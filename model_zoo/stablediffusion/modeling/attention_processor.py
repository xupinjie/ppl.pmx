...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if norm_num_groups is not None:
            self.group_norm = GroupNorm(num_channels=query_dim, num_groups=norm_num_groups, eps=eps)
        else:
        ...
        self.to_q =  ColumnParallelLinear(proc_group, query_dim, self.inner_dim,
                                          bias_term=bias, gather_output=False)

        if not self.only_cross_attention:
            # only relevant for the `AddedKVProcessor` classes
            # self.to_k = nn.Linear(self.cross_attention_dim, self.inner_dim, bias=bias)
            # self.to_v = nn.Linear(self.cross_attention_dim, self.inner_dim, bias=bias)
            self.to_k = ColumnParallelLinear(proc_group, self.cross_attention_dim, self.inner_dim,
                                             bias_term=bias, gather_output=False)
            self.to_v = ColumnParallelLinear(proc_group, self.cross_attention_dim, self.inner_dim,
                                             bias_term=bias, gather_output=False)
        else:
            self.to_k = None
            self.to_v = None
        ...
        self.to_out.append(RowParallelLinear(proc_group, self.inner_dim, query_dim,
                                             bias_term=bias, input_is_parallel=True))
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if input_ndim == 4:
            ...
            hidden_states = PMX.reshape(hidden_states, [0, channel, -1]).transpose(1, 2)
        ...
        # pmx.mha transpose the tensor inside
        ...
        query = PMX.reshape(query, [0, -1, attn.heads, head_dim])
        key = PMX.reshape(key, [0, -1, attn.heads, head_dim])
        value = PMX.reshape(value, [0, -1, attn.heads, head_dim])

        # import ipdb;ipdb.set_trace()
        hidden_states = PMX.multi_head_attention(query, key, value,
        ...
        # pmx.mha has transposed the tensor inside
        # hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        # hidden_states = hidden_states.reshape(batch_size, -1, attn.heads * head_dim)
        hidden_states = PMX.reshape(hidden_states, [0, -1, attn.heads * head_dim])


        # default cast in pmx.mha
